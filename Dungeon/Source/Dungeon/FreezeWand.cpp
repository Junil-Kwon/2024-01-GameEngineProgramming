...
void AFreezeWand::OnStart() {
	Super::OnStart();
}
void AFreezeWand::OnSpawn() {
	Super::OnSpawn();
}
void AFreezeWand::OnDespawn() {
	Super::OnDespawn();
}
...
bool AFreezeWand::VerifyAction(Action value) {
	if (value == Action::Attack && GetAction() == Action::Defend) return false;
	return true;
}
void AFreezeWand::UpdateAction(float DeltaTime) {
	...
	case Action::Attack:
		...
		if (GetActionDelay() - DeltaTime == 0.0f) {
			...
			if (GetEnergeCost() <= parent->GetEnergy()) {
				...
				AEntity* projectile = Spawn(Identifier::Icicle, location);
